	...
	socket.on('sat', function(message) {
		if (user && user.permissions && user.permissions.chat) {
			if (message.type == 'word') {
				...
				connection.query("SELECT COUNT(*) FROM sat_questions", function(err, result) {
					var indexes = [];
					indexes.push(' id=' + (Math.floor(Math.random() * result[0]['COUNT(*)'] + 1)));
					connection.query('SELECT question,answer,number_of_answers FROM sat_questions WHERE ' + indexes.join(' or '), function(error, results) {
						var answers = [];
						answers.push(results[0].answer);
						connection.query("SELECT COUNT(*) FROM sat_words", function(err, result1) {
							var indexes = [];
							for (var i = 0; i < numberOfAnswers * results[0].number_of_answers; i++) {
								indexes.push(' id=' + (Math.floor(Math.random() * result1[0]['COUNT(*)'] + 1)));

							}
							connection.query('SELECT word FROM sat_words WHERE ' + indexes.join(' or '), function(error, results1) {
